...
for i in $VERSIONS; do
  ...
  if vercmp "$i" "==" "8.18*"; then
    cp -a ../../../reference-from-coq_8.18-8.5/debian ./ || exit $?
  elif vercmp "$i" "==" "8.17*"; then
  ...
  elif vercmp "$i" "==" "8.12*"; then
    cp -a ../../../reference-from-coq_8.12-8.5/debian ./ || exit $?
  elif vercmp "$i" "==" "8.11*"; then
