...
for TARGET in focal; do export TARGET; ./04-make-debian-from-reference.sh && ./05-build-debian-only-source${NO_ORIG_SOURCE}.sh && ./06-try-dput.sh; done && ./bump-version.sh && git commit -am "Bump version" && git push
